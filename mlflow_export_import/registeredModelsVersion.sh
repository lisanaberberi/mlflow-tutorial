...
read -p "Enter the model name to export: " MODEL_NAME
# Prompt user for model versions
read -p "Enter the model version(s) to export (separate multiple versions with spaces): " -a MODEL_VERSIONS

# Join model versions with commas
EXPORT_VERSIONS=$(IFS=,; echo "${MODEL_VERSIONS[*]}")

# Prompt user for export directory or use the default
read -p "Enter the local temporary imp/exp directory (Press Enter for default: $DEFAULT_EXPORT_DIR): " EXPORT_OUTPUT_DIR_INPUT
...
export-model --model $MODEL_NAME --output-dir $EXPORT_OUTPUT_DIR
export-model --model $MODEL_NAME --output-dir $EXPORT_OUTPUT_DIR --versions $EXPORT_VERSIONS
...
read -p "Enter the model name to import: " MODEL_NAME
read -p "Enter the experiment name to import: " EXPERIMENT_NAME
# Prompt user for import directory or use the default
read -p "Enter the local temporary imp/exp directory (Press Enter for default: $DEFAULT_EXPORT_DIR): " IMPORT_INPUT_DIR_INPUT
IMPORT_INPUT_DIR="${IMPORT_INPUT_DIR_INPUT:-$DEFAULT_EXPORT_DIR}"

# Import registered model
import-model --model $MODEL_NAME  --experiment-name $EXPERIMENT_NAME  --input-dir $IMPORT_INPUT_DIR --versions $EXPORT_VERSIONS
